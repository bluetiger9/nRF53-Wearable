...
bool MAX30105::begin(const device *wirePort, uint8_t i2c_addr) {
  ...
  if (readPartID() != MAX_30105_EXPECTEDPARTID) {
    printk("MAX3010x init failed!\n");
    // Error -- Part ID read from MAX30105 does not match expected part ID.
    // This may mean there is a physical connectivity problem (broken wire, unpowered, etc).
    return false;
  }

  printk("MAX3010x init OK!\n");
...
void MAX30105::_i2c_write(uint8_t data) {
  this->_i2c_write(&data, 1);
}

void MAX30105::_i2c_write(uint8_t *data, uint8_t len) {
  i2c_write(_i2cPort, data, len, MAX30105_ADDRESS);
}
...
uint16_t MAX30105::check(void)
{
  ...
  if (readPointer != writePointer)
  {
    ...
    this->_i2c_write(MAX30105_FIFODATA);
    ...
    while (bytesLeftToRead > 0)
    {
      ...
      uint8_t buff[I2C_BUFFER_LENGTH];
      uint8_t bufPos = 0;

      //Request toGet number of bytes from sensor
      i2c_read(_i2cPort, buff, toGet, MAX30105_ADDRESS);
      ...
      while (toGet > 0)
      {
        ...
        temp[3] = 0;
        temp[2] = buff[bufPos]; bufPos++; //_i2cPort->read();
        temp[1] = buff[bufPos]; bufPos++; //_i2cPort->read();
        temp[0] = buff[bufPos]; bufPos++; //_i2cPort->read();
...
uint8_t MAX30105::readRegister8(uint8_t address, uint8_t reg) {
  ...
  uint8_t res, err;
  err = i2c_reg_read_byte(_i2cPort, MAX30105_ADDRESS, reg, &res);
  if (err > 0) {
    return (0); //Fail
  }
  return res;
...
void MAX30105::writeRegister8(uint8_t address, uint8_t reg, uint8_t value) {
  uint8_t err;
  err = i2c_reg_write_byte(_i2cPort, MAX30105_ADDRESS, reg, value);
  if (err > 0) {
    // TODO: handle error
  }
  //_i2cPort->beginTransmission(address);
